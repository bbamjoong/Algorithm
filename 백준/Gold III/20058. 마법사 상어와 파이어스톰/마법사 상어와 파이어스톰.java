...
public class Main {
    ...
    static void findBiggestIce() { // bfs
        // 칸의 최대 숫자는 100이고 얼음은 녹기만 함. 따라서 관계없는 값인 101을 이용해 visited 배열을 생성하지 않고
        // 메모리를 아껴본다.
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (matrix[i][j] == 0) { // 얼음이 없는 칸이면
                    matrix[i][j] = 101; // 방문 처리 후 continue
                    continue;
                }
                if (matrix[i][j] != 101) { // 방문안했으면 bfs돌림
                    matrix[i][j] = 101; // 방문 처리
                    bfs(i, j); // bfs 시작
                }
            }
        }
    ...
    static void bfs(int i, int j) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                if (matrix[nx][ny] != 101) { // 마지막 조건으로, 방문하지 않은 곳이면
                    cnt++; // 얼음덩어리의 카운트를 추가해준다.
                    matrix[nx][ny] = 101; // 방문처리
